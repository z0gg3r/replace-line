...
fn main() {
    ...
    let mut opts = getopt::Parser::new(&args, "f:r:h");
    ...
    loop {
            ...
            .expect("ERROR 3: Could not parse arguments!")
        {
            ...
            Some(opt) => match opt {
                ...
                Opt('r', Some(arg)) => pattern = arg.clone(),
    ...
    if help || pattern.is_empty() {
        ...
        if !input.is_empty() {
            let v = replace(split(input), pattern.clone());
...
fn replace(src: Vec<String>, pattern: String) -> Vec<String> {
    let mut v: Vec<String> = vec![];
...
fn read_file(_path: String) -> Result<String, Box<dyn std::error::Error + 'static>> {
